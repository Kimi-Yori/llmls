...
カラーコード、桁揃え、アイコン装飾なし。ファイル名、サイズ、経過時間、git status、必要時だけの権限メタデータを最小トークンで出力します。Claude Code、Cursor、Codex、その他あらゆるLLMコーディングエージェント向けに設計されています。

## なぜ llmls？

`ls -la` は多くの場合、LLMに不要な情報でトークンを浪費します：
...
パーミッション、オーナー、グループ、リンク数、`.git/`エントリ — デフォルトの偵察ステップではノイズです。llmlsはデフォルトで省きます：
...
権限確認が必要な時は `-l` を追加すると、有用なlongメタデータだけを戻します：
...
- **時刻ソート** — `-t` でmtime降順に切り替え
- **必要時だけのlongメタデータ** — `-l` でoctal権限、owner、group、symlink targetを追加
...
- `-l` 指定時、modeはoctal文字列。owner/groupの名前解決に失敗した場合は数値IDにフォールバック
...
`-t` 指定時はmtime降順でソートします。既存のスマート優先度は、同一timestampのタイブレークとしてのみ使います。
...
| `-d`, `--dense` | 圧縮フォーマット |
| `-j`, `--json` | JSON出力 |
| `-l`, `--long` | octal mode、owner、group、symlink targetを表示 |
| `-t`, `--time` | mtime降順でソート |
